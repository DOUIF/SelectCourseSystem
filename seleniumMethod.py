...
class Select_Course_Robot(threading.Thread):
    ...
    def run(self):
        ...
        account = WebDriverWait(self.driver, 10).until(
            lambda driver: self.driver.find_element_by_css_selector("#UserAccount"))
        account.send_keys(self.account)
        # 輸入密碼
        password = WebDriverWait(self.account, 10).until(
            lambda driver: self.driver.find_element_by_css_selector('#Password'))
        password.send_keys(self.password)
        # 登入按鈕
        login = WebDriverWait(self.driver, 10).until(
            lambda driver: self.driver.find_element_by_css_selector('#Login'))
        login.click()

        # 進入選課畫面
        self.driver.get(
            "https://aais6.nkust.edu.tw/selcrs_std/AddSelect/AddSelectPage")
        time.sleep(1)
    ...
    def Select_Course_Loop(self):
        ...
        search = WebDriverWait(self.driver, 10).until(
            lambda driver: self.driver.find_element_by_css_selector('#scr_selcode'))
...
def main():
    ...
    for course in courses:
        robot = Select_Course_Robot(
            accountJson["account"], accountJson["password"], course, 45 * len(courses))
        robots.append(robot)        
        robot.start()
        time.sleep(45)
